...
describe("attendance flow while editing an existing roster", () => {
  ...
  it("keeps legacy public editor links working before the owner signs in to migrate data", async () => {
    const t = convexTest(schema, modules);
    const publicEditor = t.withIdentity({
      subject: "clerk|public-editor",
      tokenIdentifier: "token-public-editor",
    ...
    await t.run(async (ctx) => {
      ...
      const sessionId = await ctx.db.insert("sessions", {
        ...
        date: "2026-03-27",
        isOpen: true,
        editorToken: "legacy-editor-token",
        createdAt: 1,
      });
    ...
    const editorSession = await publicEditor.query(api.attendance.getEditorSessionByToken, {
      token: "legacy-editor-token",
    });

    expect(editorSession?.students).toHaveLength(1);
    expect(editorSession?.students[0]).toMatchObject({
      studentId: "1001",
      displayName: "Alice Able",
      present: false,
    });

    const legacyStudentRef = editorSession?.students[0]?.studentRef;
    if (!legacyStudentRef) {
      throw new Error("Expected legacy student ref.");
    }

    await publicEditor.mutation(api.attendance.toggleByEditorToken, {
      token: "legacy-editor-token",
      studentRef: legacyStudentRef,
      clientNow: 1_742_000_000_000,
    });

    const updatedEditorSession = await publicEditor.query(api.attendance.getEditorSessionByToken, {
      token: "legacy-editor-token",
    });
    expect(updatedEditorSession?.students[0]).toMatchObject({
      studentId: "1001",
      present: true,
      markedAt: 1_742_000_000_000,
    });
