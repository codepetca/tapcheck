...
export function RosterDetailPageClient({
  rosterId,
  viewerEmail,
}: {
  rosterId: string;
  ...
  const data = useQuery(api.rosters.getById, {
    rosterId: rosterId as Id<"rosters">,
  });
  ...
  const activeSessionId =
    data && data !== null ? (data.sessions.find((session) => session.isOpen)?._id ?? null) : null;
  const sessionExport = useQuery(
    api.attendance.getSessionExport,
    latestSessionId ? { sessionId: latestSessionId } : "skip",
  );
  const editorShare = useQuery(
    api.sessions.getEditorLink,
  ...
  const editorPath = editorShare ? buildEditorPath(editorShare.editorToken) : "";
  const editorUrl =
    editorShare && typeof window !== "undefined"
  ...
  const attendanceRows = sessionExport?.rows ?? [];
  const attendanceByStudentId = new Map<string, boolean>(
    attendanceRows.map((row) => [row.studentId, row.present] as const),
  );
        ...
        <div>
          {isEditingTitle ? (
            ...
            <div>
              ...
              {viewerEmail ? (
                <p className="mt-1 text-left text-xs font-normal tracking-normal text-slate-500">
                  {viewerEmail}
                </p>
              ) : null}
            </div>
          ...
          {activeSession ? (
            <div className="grid w-full grid-cols-2 gap-3">
              <button
                ...
                className="inline-flex h-[68px] w-full items-center justify-center gap-2 rounded-full bg-rose-600 px-4 text-sm font-medium text-white transition hover:bg-rose-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-400 disabled:cursor-not-allowed disabled:bg-rose-200 sm:px-5"
              >
              ...
              <button
                ...
                disabled={!editorShare}
                aria-label={editorLinkControlLabel}
                title={editorLinkControlLabel}
                className="inline-flex h-[68px] w-full items-center justify-center gap-2 rounded-full border border-emerald-200 bg-white/90 px-4 text-sm font-medium text-emerald-950 transition hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400"
              >
                {canShareEditorLink ? (
                ...
                <span className="hidden sm:inline">
                  {canShareEditorLink ? "Send link" : copiedLink ? "Copied" : "Collection Link"}
                </span>
              </button>
            </div>
          ) : (
            <button
              type="button"
              onClick={() => void (latestSession ? handleResumeSession() : handleStartSession())}
              disabled={isStartingSession || !hasStudents}
              aria-label={startControlLabel}
              title={startControlLabel}
              className="inline-flex h-[68px] w-full items-center justify-center gap-2 rounded-full bg-emerald-600 px-4 text-sm font-medium text-white transition hover:bg-emerald-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 disabled:cursor-not-allowed disabled:bg-emerald-200 sm:px-5"
            >
